...
namespace NFinal.Compile.Template
{
    public class Razor
    {
        public void GetAspxCode(StringBuilder sb, IEnumerable<System.Web.Razor.Parser.SyntaxTree.SyntaxTreeNode> trees)
        {
        ...
        {
            return Frame.MapPath(path);
        }
        public string TranslateTemplate(string templateString,int deepth)
        {
            string renderPagePattern = @"@RenderPage\s*\(\s*""(~?[_a-zA-Z0-9\./]+)""\s*(?:,\s*ViewBag.([_a-zA-Z0-9]+)\s*)?\)\s*;?";
            ...
            if (mac.Count > 0 && deepth<6)
            {
                ...
                foreach (Match mat in mac)
                {
                    if (mat.Success)
                    {
                        ...
                        subTemplateString = DeleteViewBagDeclaration(subTemplateString);
                        sr.Close();
                        if (mat.Groups[2].Success)
                        {
                            controllName = mat.Groups[2].Value;
                            subTemplateString = subTemplateString.Replace("ViewBag.", controllName + ".");
                        }
        ...
        {
            int deepth = 0;
            template= DeleteViewBagDeclaration(template);
            deepth = 0;
            template = TranslateTemplate(template, deepth);
            template = TranslateToAspxCode(template);
            if (compress)
            {
        ...
        public string TranslateToAspxCode(string templateString)
        {
            ...
            StringBuilder sb = new StringBuilder();
            //取得转换成aspx的代码
            GetAspxCode(sb, results.Document.Children);
