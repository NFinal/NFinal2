...
namespace NFinal.Advanced
{
    partial class ByteArrayUtil
    {
        ...
        public static uint ToUInt32(this byte[] bytes, int start, bool isLittleEndian)
        {
            return ToValue<uint>(bytes, start, isLittleEndian, BitConverter.ToUInt32, 4);
        }
        ...
        /// <exception cref="ArgumentNullException">字节数据为null</exception>
        /// <exception cref="ArgumentException"><c>bytes</c>从<c>start</c>开始不足4个字节</exception>
        public static uint ToUInt32(this byte[] bytes, int start = 0)
        {
            return ToUInt32(bytes, start, Settings.Global.IsLittleEndian);
        }
        ...
        public static uint[] ToUInt32(this byte[] bytes, int start, int length, bool isLittleEndian)
        {
            return ToValueArray<uint>(bytes, start, length, isLittleEndian, BitConverter.ToUInt32, 4);
        }
        ...
        /// <exception cref="ArgumentException">start小于0，length小于0，
        /// 或参与转换的字节数不是0或4的倍数</exception>
        public static uint[] ToUInt32(this byte[] bytes, int start, int length)
        {
            return ToUInt32(bytes, start, length, Settings.Global.IsLittleEndian);
        }
        ...
        public static byte[] ToByteArray(this uint value, bool isLittleEndian)
        {
        ...
        /// <param name="isLittleEndian">true表示最大有效字节在右端，false表示最大有效字节在左端</param>
        /// <returns>字节数组表示的数据</returns>
        public static byte[] ToByteArray(this uint[] array, bool isLittleEndian)
        {
            const int STEP = 4;
