...
namespace NFinalRazorGenerator
{
    public class Razor
    {
        ...
        public void GetAspxCode(StringBuilder sb, ref string modelType, List<string> usings, IEnumerable<System.Web.Razor.Parser.SyntaxTree.SyntaxTreeNode> trees)
        {
            ...
            foreach (var tr in trees)
            {
                if (tr.IsBlock)
                {
                    ...
                    GetAspxCode(sb, ref modelType, usings, block.Children);
                }
                else
                {
                    ...
                    switch (span.Kind)
                    {
                        case System.Web.Razor.Parser.SyntaxTree.SpanKind.Code:
                            if (span.Parent.Type == System.Web.Razor.Parser.SyntaxTree.BlockType.Expression)
                            {
                                ...
                                {
                                    modelType = span.Next.Content.Split('\r','\n')[0];
                                }
                                else
                                {
                                    sb.AppendLine(string.Format("<%={0}%>", span.Content));
                                }
                            }
                            else if (span.Parent.Type == System.Web.Razor.Parser.SyntaxTree.BlockType.Directive)
                            {
                                if (span.CodeGenerator != null)
                                {
                                    Type t = span.CodeGenerator.GetType();
                                    if (t == typeof(System.Web.Razor.Generator.AddImportCodeGenerator))
                                    {
                                        var generator = span.CodeGenerator as System.Web.Razor.Generator.AddImportCodeGenerator;
                                        usings.Add(generator.Namespace);
                                    }
                                    else if (t == typeof(System.Web.Razor.Generator.SetBaseTypeCodeGenerator))
                                    {
                                        var generator = span.CodeGenerator as System.Web.Razor.Generator.SetBaseTypeCodeGenerator;
                        ...
                        case System.Web.Razor.Parser.SyntaxTree.SpanKind.Markup:
                            ...
                            if (span.Previous?.Content == "model")
                            {
                            }
                            else
                            {
