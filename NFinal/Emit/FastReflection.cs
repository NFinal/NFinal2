...
/// <summary>
/// 这是国外人写的一个快速反射类，修正了其中的GetHashCode函数，以及Cache存取bug.
/// 此类库以MIT协议开源，可任意修改并用于任何用途。
/// </summary>
namespace NFinal.Emit
{
    ...
    {
        static ILEmitter emit = new ILEmitter();//如果使用int类型，则可能发生类型碰撞，导致代理失败。
        ...
        public static CtorInvoker<T> DelegateForCtor<T>(this Type type, params Type[] paramTypes)
        {
            Type key = type;
            Delegate result;
            if (cacheDelegateForCtor.TryGetValue(key, out result))
            ...
            cacheDelegateForCtor[key] = result;
            return (CtorInvoker<T>)result;
        }
        ...
        public static MemberGetter<TTarget, TReturn> DelegateForGet<TTarget, TReturn>(this PropertyInfo property)
        {
            ...
            PropertyInfo key = property;
            Delegate result;
            if (cacheDelegateForPropertyGetter.TryGetValue(key, out result))
                return (MemberGetter<TTarget, TReturn>)result;

            return GenDelegateForPropertyGetter<MemberGetter<TTarget, TReturn>>(
        ...
        public static MemberSetter<TTarget, TValue> DelegateForSet<TTarget, TValue>(this PropertyInfo property)
        {
            ...
            PropertyInfo key = property;
            Delegate result;
            if (cacheDelegateForPropertySetter.TryGetValue(key, out result))
                return (MemberSetter<TTarget, TValue>)result;

            return GenDelegateForPropertySetter<MemberSetter<TTarget, TValue>>(
        ...
        public static MemberGetter<TTarget, TReturn> DelegateForGet<TTarget, TReturn>(this FieldInfo field)
        {
            FieldInfo key = field;
            Delegate result;
            if (cacheDelegateForFieldGettter.TryGetValue(key, out result))
                return (MemberGetter<TTarget, TReturn>)result;

            return GenDelegateForFieldGetter<MemberGetter<TTarget, TReturn>>(
        ...
        public static MemberSetter<TTarget, TValue> DelegateForSet<TTarget, TValue>(this FieldInfo field)
        {
            FieldInfo key = field;
            Delegate result;
            if (cacheDelegateForFieldSettter.TryGetValue(key, out result))
                return (MemberSetter<TTarget, TValue>)result;

            return GenDelegateForFieldSetter<MemberSetter<TTarget, TValue>>(
        ...
        public static MethodCaller<TTarget, TReturn> DelegateForCall<TTarget, TReturn>(this MethodInfo method)
        {
            ...
            Delegate result;
            if (cacheDelegateForMethod.TryGetValue(key, out result))
                return (MethodCaller<TTarget, TReturn>)result;

            return GenDelegateForCall<MethodCaller<TTarget, TReturn>>(
        ...
        {
            return (((long)typeof(T).GetHashCode()) << 32) ^ member.GetHashCode();
        }

        //static TDelegate GenDelegateForMember<TDelegate, TMember>(TMember member, long key, string dynMethodName,
        ...
        //}
        static TDelegate GenDelegateForPropertyGetter<TDelegate>(PropertyInfo member, PropertyInfo key, string dynMethodName,
            Action<PropertyInfo> generator, Type returnType, params Type[] paramTypes)
            where TDelegate : class
        {
            var dynMethod = new DynamicMethod(dynMethodName, returnType, paramTypes, true);
            ...
            var result = dynMethod.CreateDelegate(typeof(TDelegate));
            cacheDelegateForPropertyGetter[key] = result;
            return (TDelegate)(object)result;
        }
        static TDelegate GenDelegateForPropertySetter<TDelegate>(PropertyInfo member, PropertyInfo key, string dynMethodName,
            Action<PropertyInfo> generator, Type returnType, params Type[] paramTypes)
            where TDelegate : class
        {
            var dynMethod = new DynamicMethod(dynMethodName, returnType, paramTypes, true);
            ...
            var result = dynMethod.CreateDelegate(typeof(TDelegate));
            cacheDelegateForPropertySetter[key] = result;
            return (TDelegate)(object)result;
        }
        static TDelegate GenDelegateForFieldGetter<TDelegate>(FieldInfo member, FieldInfo key, string dynMethodName,
            Action<FieldInfo> generator, Type returnType, params Type[] paramTypes)
            where TDelegate : class
        {
            var dynMethod = new DynamicMethod(dynMethodName, returnType, paramTypes, true);
            ...
            var result = dynMethod.CreateDelegate(typeof(TDelegate));
            cacheDelegateForFieldGettter[key] = result;
            return (TDelegate)(object)result;
        }
        static TDelegate GenDelegateForFieldSetter<TDelegate>(FieldInfo member, FieldInfo key, string dynMethodName,
            Action<FieldInfo> generator, Type returnType, params Type[] paramTypes)
            where TDelegate : class
        {
            var dynMethod = new DynamicMethod(dynMethodName, returnType, paramTypes, true);
            ...
            var result = dynMethod.CreateDelegate(typeof(TDelegate));
            cacheDelegateForFieldSettter[key] = result;
            return (TDelegate)(object)result;
        }
        static TDelegate GenDelegateForCall<TDelegate>(MethodInfo member, MethodInfo key, string dynMethodName,
            Action<MethodInfo> generator, Type returnType, params Type[] paramTypes)
            where TDelegate : class
        {
            var dynMethod = new DynamicMethod(dynMethodName, returnType, paramTypes, true);
            ...
            var result = dynMethod.CreateDelegate(typeof(TDelegate));
            cacheDelegateForMethod[key] = result;
            return (TDelegate)(object)result;
        }
