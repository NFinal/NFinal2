...
namespace NFinal.Config
{
    public class Configration
    {
        ...
        public static void Init(Middleware.MiddlewareConfigOptions options)
        {
            ...
            {
                assembly = Assembly.Load(new AssemblyName(options.plugs[i].assemblyName));
                modules = assembly.GetModules();
                for (int j = 0; j < modules.Length; j++)
                {
                    types = modules[j].GetTypes();
                    var attributes= types[j].GetTypeInfo().GetCustomAttributes(typeof(ConfigAttribute),true);
