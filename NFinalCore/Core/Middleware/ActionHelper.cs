...
namespace NFinal.Middleware
{
    ...
    public class ActionHelper
    {
        ...
        public static void GetControllerUrl(out string controllerName, out string areaName, out string subDomain, Type controller, NFinal.Middleware.MiddlewareConfigOptions options)
        {
            ...
            controllerAttributes = (ControllerAttribute[])controller.GetTypeInfo().GetCustomAttributes(typeof(ControllerAttribute), true);
            ...
            areaAttributes = (AreaAttribute[])controller.GetTypeInfo().GetCustomAttributes(typeof(AreaAttribute), true);
            ...
            subDomainAttributes = (SubDomainAttribute[])controller.GetTypeInfo().GetCustomAttributes(typeof(SubDomainAttribute), true);
            ...
            MethodInfo methodInfo, NFinal.Middleware.MiddlewareConfigOptions options)
        {
            ...
            else if (options.urlRouteRule == UrlRouteRule.AreaControllerCustomActionUrl)
            {
                ...
                {
                    if (attr.GetType().GetTypeInfo().IsSubclassOf(typeof(UrlAttribute)))
                    {
        ...
        public static TAttribute[] GetFilters<TAttribute>(Type filterType, Type controller, MethodInfo action)
        {
            List<TAttribute> filterList = new List<TAttribute>();
            var controllerFilters = controller.GetTypeInfo().GetCustomAttributes(true).ToArray();
            for(int i=0;i<controllerFilters.Count();i++)
            {
                if (filterType.IsAssignableFrom(controllerFilters[i].GetType()))
                {
                    filterList.Add((TAttribute)(object)controllerFilters[i]);
                }
            }
            var actionFilters = action.GetCustomAttributes(true).ToArray();
            for (int i = 0; i < actionFilters.Length; i++)
            {
                if (filterType.IsAssignableFrom(actionFilters[i].GetType()))
                {
                    filterList.Add((TAttribute)(object)actionFilters[i]);
                }
            }
